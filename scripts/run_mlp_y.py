...
# central config
def run_with_config(mlp_y_config, seed=1, device=None):
    """Run the MLP-year full pipeline using the provided `mlp_y_config` dict.

    This function does not import `master_config` and is safe to call from a
    notebook where the notebook defines `MLP_Y_CONFIG`.
    """
    data_dir = mlp_y_config["dataset_path"]
    # Look for both .csv and .pt files
    file_item_list = []
    for f in os.listdir(data_dir):
        if (
            mlp_y_config["file_pattern"] in f
            and mlp_y_config.get("file_pattern_suffix", "") in f
            and (f.endswith(".csv") or f.endswith(mlp_y_config.get("file_extension", ".pt")))
        ):
    ...
    for file_item in file_item_list:
        ...
        data, _, _ = norm_mlp_tensor(data, mlp_y_config.get("freq", "year"))
        labels, min_value, max_value = norm_mlp_tensor(
            labels, mlp_y_config.get("freq", "year")
        )

        # determine test year
        if "-07" in file_item:
            year = mlp_y_config["test_year"]["-07"]
        elif "13-19" in file_item:
            year = mlp_y_config["test_year"]["13-19"]
        else:
            year = mlp_y_config["test_year"]["default"]

        train_data, test_data, train_targets, test_targets = split_mlp_dataset_by_year(
            data, labels, year, freq=mlp_y_config.get("freq", "year")
        )

        set_seed(seed)

        # use centralized param grid
        param_grid = mlp_y_config["param_grid"]
        ...
        best_params, best_overall_loss = hyperparameter_search(
            ...
            train_targets,
            param_grid,
            k_folds=mlp_y_config.get("k_folds", 5),
            device=device,
        )
        ...
        model_path = os.path.join(
            mlp_y_config.get("checkpoint_dir", "checkpoints_mlp/"),
...
def _main():
    ...
    try:
        from master_config import MLP_Y_CONFIG, SEED, CUDA_VISIBLE_DEVICES

        os.environ["CUDA_VISIBLE_DEVICES"] = CUDA_VISIBLE_DEVICES
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        run_with_config(MLP_Y_CONFIG, seed=SEED, device=device)
...
def train_and_evaluate(
    ...
    no_train_loss_res, no_train_gdp_loss_res = no_train_loss(
        model, train_loader, criterion, "cuda"
    )
    print(
        "initial train loss: ",
        no_train_loss_res,
    ...
    no_train_loss_res, no_train_gdp_loss_res = no_train_loss(
        model, val_loader, criterion, "cuda"
    )
    print(
        "initial val loss: ",
        no_train_loss_res,
...
def train_and_evaluate_final(
    train_data, test_data, train_targets, test_targets, best_params, device="cpu"
):
    ...
    no_train_loss_res, no_train_gdp_loss_res = no_train_loss(
        final_model, train_dataloader, final_criterion, "cuda"
    )
    print(
        "initial train loss: ",
        no_train_loss_res,
    ...
    no_train_loss_res, no_train_gdp_loss_res = no_train_loss(
        final_model, test_dataloader, final_criterion, "cuda"
    )
    print(
        "initial val loss: ",
        no_train_loss_res,
    ...
    # # 保存最终模型
    model_save_path = (
        "checkpoints_mlp/" + file_item.replace(".pt", "_") + "mlp_best_final_model.pth"
    )
