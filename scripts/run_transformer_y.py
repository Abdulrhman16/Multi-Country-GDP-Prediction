...
# Do not set CUDA_VISIBLE_DEVICES at import time; notebooks control environment.
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=500, batch_size=64, dropout=0.1):
        ...
        pe = pe.unsqueeze(0).repeat(
            batch_size, 1, 1
        )  # 调整形状为 [batch_size, max_len, d_model]
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Linear(
            input_dim, embed_dim - repeat_dim
        )  # Embedding from input_dim to embed_dim
        ...
        )  # To output a single real number

    def forward(self, src, src_mask):
        # concat_tensor = src[:, :, -1:].repeat(1,1,self.repeat_dim)
        concat_tensor = torch.repeat_interleave(
            src[:, :, -1:], repeats=self.repeat_dim, dim=-1
        )
    ...
    for param_values in tqdm(param_combinations, desc="Hyperparameter Search"):
        ...
        if avg_val_loss < best_overall_loss:
            ...
            model_save_path = (
                ...
                + file_item.replace(".pt", "_")
                + "transformer_best_valid_model.pth"
            )
...
def train_and_evaluate_final(train_dataset, test_dataset, best_params, device="cpu"):
    ...
    no_train_loss_res = no_train_loss(
    ...
    model_save_path = (
        "checkpoints_transformer/"
        + file_item.replace(".pt", "_")
        + "transformer_best_final_model.pth"
    )
...
def run_with_config(transformer_config, file_item_list=None, seed=1, device=None):
    if file_item_list is None:
        # default set from config path (assumes dataset folder contains matching files)
        data_dir = transformer_config["dataset_path"]
        file_item_list = [
            f for f in os.listdir(data_dir) if transformer_config["file_pattern"] in f
        ]
    ...
    for file_item in tqdm(file_item_list[:]):
        ...
        train_dataset_path = os.path.join(
            transformer_config.get("dataset_path", "dataset_RT"), file_item
        )
        test_dataset_path = train_dataset_path.replace("train_", "test_")
        ...
        # use config param_grid if provided, otherwise fallback
        param_grid = transformer_config.get(
            "param_grid",
            {
        ...
        best_params, best_overall_loss = hyperparameter_search(
            train_dataset,
            param_grid,
            k_folds=transformer_config.get("k_folds", 5),
            device=device,
        )
        ...
        best_params = train_and_evaluate_final(
            train_dataset, test_dataset, best_params, device
        )

        model_path = os.path.join(
            transformer_config.get("checkpoint_dir", "checkpoints_transformer/"),
...
def _main():
    try:
        from master_config import TRANSFORMER_Y_CONFIG, SEED, CUDA_VISIBLE_DEVICES

        os.environ["CUDA_VISIBLE_DEVICES"] = CUDA_VISIBLE_DEVICES
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        run_with_config(TRANSFORMER_Y_CONFIG, seed=SEED, device=device)
