...
def run(config, seed=1, dataset_path="dataset", output_file="linear_mlp_q_res.csv"):
    """Run linear MLP quarter baseline with provided config."""
    res_list = []
    for file_item in os.listdir(dataset_path):
        if not ("MLP_data_" in file_item and "_q_" in file_item):
            continue
        print(file_item)
        temp_dict = {}
        ...
        label_path = os.path.join(
            dataset_path, file_item.replace("MLP_data", "MLP_label")
        )
